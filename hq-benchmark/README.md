...
Die neue Ansicht nutzt fest `/v2/Companies` und `/v2/Documents`. Die öffentliche v2-Anleitung beschreibt die Felder für Belegart, Status, Rechnungsdatum, Firmenbezug und Netto-Betrag nicht vollständig. Daher zuerst **HQ-Felder prüfen** anklicken. Die Oberfläche zeigt Feldnamen, Datentypen und kurze mögliche Belegart-/Statuswerte; sie speichert keine echten Antwortinhalte. Die vorgeschlagenen Feldnamen kontrollieren und anhand eines bekannten Kunden und Belegs bestätigen. Rechnungsarten, Gutschriftarten und nur die tatsächlich umsatzwirksamen Statuswerte eintragen. Optional einen Kundentyp wählen; ohne ihn umfasst die Liste alle Firmenarten. Ein Währungsfeld sollte gesetzt werden, damit andere Währungen nicht mit EUR summiert werden. Stimmen Felder oder Werte nicht, ist das Ergebnis fachlich falsch; deshalb vor einem weiteren Lasttest gegen HQ abgleichen.
...
JavaScript-Syntax und Manifest wurden lokal geprüft. Die elf Tests des ursprünglichen Benchmarks und sechs Tests der Umsatzansicht mit nachgebildeten Apps-Script-Diensten sind bestanden, unter anderem Zugriffsprüfung, GET-only, Seitennavigation, Rechnung/Gutschrift, Fehlerfälle und Batch-Metriken. Der ursprüngliche Firmenabruf wurde real ausgeführt; die neue Umsatzansicht braucht noch den Test gegen die tatsächlichen HQ-Belegfelder.
