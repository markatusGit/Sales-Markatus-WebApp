...
2. Klicke bei **Dateien** auf das Pluszeichen und wähle **Skript**. Nenne die neue Datei `Revenue` (Google ergänzt `.gs`). Kopiere den vollständigen Inhalt aus [Revenue.gs](../hq-benchmark/Revenue.gs) hinein.
...
5. Unter dem bisherigen Test erscheint **Kunden und Netto-Umsatz live laden**. Klicke zuerst **HQ-Felder prüfen**. Die Seite zeigt die in deinem HQ vorhandenen Feldnamen sowie mögliche Belegart- und Statuswerte. Wähle die passenden Felder für Firmen-ID, Firmenname, Beleg-ID, Firmenbezug, Rechnungsdatum, Netto-Gesamtbetrag, Belegart und Belegstatus. Trage ein, welche HQ-Werte Rechnungen, Gutschriften und tatsächlich gültige Belegstatus bedeuten.
6. Vergleiche diese Zuordnung mit mindestens einem bekannten HQ-Kunden samt Rechnung und Gutschrift. Erst wenn die Werte stimmen, wähle den Zeitraum und klicke **Live-Liste laden**. Die Seite zeigt nur eine vollständige Liste; bei zu vielen Seiten oder einem Fehler stoppt sie mit einer Meldung. Optional kannst du einen Kundentyp und ein Währungsfeld setzen. Ohne Kundentyp können auch Lieferanten oder andere Firmen erscheinen; ohne Währungsfeld ist EUR nicht geprüft.
7. Für den Vergleich an zwei PCs öffne dieselbe Web-App-URL auf beiden Geräten mit deinem freigeschalteten Google-Konto. Stelle auf beiden denselben Zeitraum und dieselbe Feldzuordnung ein. Klicke dann möglichst gleichzeitig **Live-Liste laden**. Lade auf beiden Rechnern das Messprotokoll herunter und gib mir die beiden JSON-Dateien. Die Protokolle enthalten keine Kundennamen oder Einzelbeträge.

Wenn eine zweite Person mit einem eigenen Google-Konto testen soll, muss ihre Adresse zuerst in `HQ_BENCH_ALLOWED_EMAILS` aufgenommen und der Web-App-Zugriff passend auf interne Nutzer erweitert werden. Diese Testseite zeigt echte Kunden- und Finanzdaten und gehört deshalb nicht in eine öffentliche Bereitstellung. Die neue Ansicht ist noch nicht gegen die tatsächlichen HQ-Belegfelder getestet; wenn die Zuordnung unklar ist, schicke mir nur die angezeigten **Feldnamen und möglichen Statuswerte**, keine Kundeninhalte und keinen Token.
