...
Der Pilotcode ist vorbereitet. Für die erste echte Messung muss dein Firebase-Projekt vollständig eingerichtet sein. Wir können die Schritte zusammen durchführen; sende **keinen HQ-Token und keinen Dienstkontoschlüssel** im Chat.
...
2. Prüfe die **Projekt-ID** unter **Projekteinstellungen → Allgemein**. Wenn dort `sales-markatus` steht, ist das die richtige ID für dieses Projekt. Die Bezeichnung allein bestätigt das noch nicht.
...
3. Prüfe in **Firestore Database → Regeln** die veröffentlichten Regeln: Unangemeldete und nicht freigeschaltete Nutzer dürfen die Pilotdaten nicht lesen; ein freigeschalteter Nutzer darf sie lesen; Browser dürfen keine Daten schreiben. Firebase bietet dafür einen Regelsimulator. Erst danach echte HQ-Daten synchronisieren.
4. Öffne die Firebase-Testseite und klicke auf **Mit Google anmelden**. Zunächst erscheint erwartbar **Kein Zugriff**. Die Seite zeigt deine **Nutzer-ID (UID)** an; diese ID kannst du mir nennen.
5. In der Firebase-Konsole unter **Firestore Database → Daten** die Sammlung `pilot_access` erstellen und darin ein Dokument mit **genau dieser UID als Dokument-ID** anlegen. Ein beliebiges harmloses Feld, z. B. `enabled = true`, genügt. Diese Sammlung kann die Testseite weder lesen noch ändern; nur Projekt-Administratoren pflegen sie. Für einen zweiten Vertriebler dessen UID genauso eintragen. Dieselbe Google-Anmeldung an zwei PCs verwendet dieselbe UID.
6. Öffne die bekannte **HQ-Testseite** und klicke im Bereich **Firebase-Vergleich** auf **1 · HQ nach Firebase synchronisieren**. Warte auf „HQ-Abgleich abgeschlossen“ und prüfe Kundenzahl und Belegzähler. Der Datenstand wird erst nach dem vollständigen Schreiben freigegeben. Der erste Abgleich simuliert den späteren Nachtlauf.
7. Klicke dort auf **2 · Firebase-Vertriebsansicht öffnen**. Öffne diese Seite auch am zweiten PC, stelle denselben Zeitraum ein und klicke auf beiden Geräten **Frisch aus Firebase laden**. Lade auf beiden das **Messprotokoll** herunter. Es enthält Zeiten und Zähler, keine Kundennamen oder Beträge. Wenn der Abgleich fachlich und technisch stimmt, im Apps-Script-Editor einmal `installFirebasePilotNightlySync` ausführen. Der tägliche Lauf ist ungefähr um 03:00 Uhr, nicht minutengenau.

**Sicherheit dieses Piloten:** Übertragen werden HQ-Firmen-ID, Kundenname und tägliche Netto-Umsätze. Telefonnummern, E-Mail-Adressen, Kontakte und Belegdetails werden derzeit **nicht** nach Firestore kopiert. Firestore beginnt im gesperrten Modus; die veröffentlichten Regeln lassen nur verifizierte und einzeln freigeschaltete Google-Nutzer lesen. Browser-Schreibzugriffe sind gesperrt. Ein freigeschalteter Nutzer erhält allerdings die gesamte Pilotliste und könnte sie kopieren. Für spätere Telefonnummern und unterschiedliche Vertriebsrollen braucht die App deshalb eine genauere Daten- und Rechteaufteilung. Interne Google-Konten sollten mit Zwei-Faktor-Anmeldung geschützt sein. Der private Dienstkontoschlüssel liegt im Apps-Script-Projekt: dessen Bearbeitungszugang auf wenige Verantwortliche beschränken, Schlüssel bei Verlust sperren und nie ins Repository übernehmen. Vor produktiver Nutzung Datenschutzvereinbarung, Zugriffsrechte und Löschfristen mit den Verantwortlichen prüfen. [Firebase-Datenschutz](https://firebase.google.com/support/privacy) · [Firestore-Verschlüsselung](https://docs.cloud.google.com/firestore/native/docs/server-side-encryption)

**Kosten und Grenze:** Für einen kleinen Test kann der Spark-Tarif reichen; Firestore bietet derzeit 1 GiB Speicher, 50.000 Lese- und 20.000 Schreibvorgänge pro Tag ohne Berechnung. Hosting und Anmeldung haben eigene Grenzen. Der Pilot liest pro neuem Seitenaufruf einen Zeiger und mehrere Datenblöcke; die Zahl der Datenblöcke zeigt er an. Die Sicherheitsregel prüft zusätzlich eine Freischaltungsdatei, was weitere abrechenbare Lesevorgänge auslösen kann. Kontingente und Preise vor einer breiten Nutzung nochmals prüfen. Firebase beschleunigt den Vertriebsabruf nur, wenn der HQ-Abgleich zuvor erfolgreich lief und der Datenstand aktuell genug ist. [Firestore-Kontingente](https://firebase.google.com/docs/firestore/quotas) · [Firebase-Preise](https://firebase.google.com/pricing)
