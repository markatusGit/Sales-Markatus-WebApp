...
### 26.09.2026 – Neue Firma und Ansprechpartner sofort in Firebase, danach gemeinsam nach HQ

**Korrektur durch den Nutzer:** Die Anweisung, den alten bereits in HQ angelegten Testfirmenauftrag fortzusetzen, hat praktisch nicht funktioniert und ist nicht der gewünschte Testfall. Der Nutzer will eine **neue** Firma samt Ansprechpartner in einem Formular anlegen, beide sofort in der gemeinsamen Firebase-Datenbank und in der App sehen und erst danach nacheinander nach HQ übertragen. Der HQ-Abgleich darf gegebenenfalls einige Minuten dauern. Die frühere r5-Abnahmeannahme für den alten Auftrag ist damit ausdrücklich zurückgestellt; es gibt weiterhin keinen Live-Nachweis einer erfolgreichen HQ-Ansprechpartneranlage.

**Lokal implementiert (r6):** Das Neuanlageformular verlangt nun Vor- und Nachname des ersten Ansprechpartners, damit eingegebene Kontaktdaten nicht stillschweigend verloren gehen. Das Speichern legt Firma und Kontakt gemeinsam im serverseitigen Firebase-Entwurf ab, ohne HQ zu berühren. Der Datensatz erhält eine stabile lokale Kennung und erscheint direkt in **Kunden**, **Ansprechpartner** und der lokalen Kundenkarte. Nach einem bewussten Start kann ein einzelner Übertragungsaufruf die Firma anlegen, anhand ihrer HQ-ID und eindeutigen Testkennung zurücklesen, den Kontakt mit genau dieser Firmen-ID anlegen, ihn zurücklesen und die technische Firmenkennung bereinigen. Nicht schlüsselrelevante Firmenfelder werden nach der Kontaktanlage geprüft, sodass ein abweichend normalisiertes Feld die sichere Kontaktzuordnung nicht schon vorher verhindert. Bei verzögerter Rücklesung bleibt die bestätigte Phase gespeichert; solange die Web-App offen ist, versucht sie bis zu sechs Mal nach je 30 Sekunden weiter. Nach Schließen kann derselbe Auftrag fortgesetzt werden. Ein unklarer Schreibausgang bleibt gesperrt, bis ein reiner HQ-Abgleich ihn eindeutig klärt. Es gibt weiterhin keine automatische nächtliche Serverausführung.

**Prüfung und Grenze:** 46 synthetische Sales-Prüfungen bestanden. Sie decken sofortige Firebase-Sichtbarkeit ohne HQ-Aufruf, Pflichtkontakt, die Reihenfolge Firma → Kontakt, Verknüpfung mit der zurückgegebenen HQ-ID, verzögerte Rücklesung, verlorene Firmen-/Kontaktantwort und Wiederaufnahme ohne doppelte POSTs sowie die Kunden- und Ansprechpartneranzeige ab. Auch nach Bestätigung bleibt die lokale Kundenkarte über ihre stabile Firebase-Kennung erreichbar. Die lokalen Apps-Script-Ausgabedateien tragen `2026-09-26-r6`. Weder der echte HQ-Kontaktaufruf noch die r6-Bereitstellung wurde durch Codex live durchgeführt; tatsächliche Annahme und Feldzuordnung im Mandanten bleiben im Test mit einer neuen eigenen TEST-Firma zu bestätigen. Schreibziele sind weiterhin ausschließlich selbst angelegte TEST-Firmen, während Ausgabe #70 nur gelesen wird.

**Nächster Nutzerschritt:** Die kleinschrittige r6-Anleitung steht oben in `sales-app/MANUELL-UEBERTRAGEN.md`. Nur `hq-benchmark/SalesBackend.gs` und `hq-benchmark/Sales.html` vollständig im bestehenden Apps-Script-Projekt ersetzen, die bisherige Bereitstellung auf **Neue Version** setzen und `2026-09-26-r6` prüfen. Danach eine einzige neue TEST-Firma mit erfundenem Ansprechpartner zunächst in Firebase speichern, ihre sofortige Anzeige in beiden App-Listen prüfen und erst anschließend auf der Firebase-Kundenkarte den HQ-Abgleich starten. Erfolg heißt: genau eine Firma und genau ein zugehöriger Ansprechpartner in HQ, Status „In HQ bestätigt“, keine technische Kennzeichnung mehr in der Firmenbeschreibung. Bei Abweichung Status/HTTP-Code mitteilen, keine zweite gleichnamige Testfirma anlegen. Keine neuen Skripteigenschaften, Manifest- oder Bereitstellungseinstellungen. Die Unterscheidung zwischen lokal vorbereitet, GitHub-Sicherung und live bereitgestellt bleibt bestehen.

**Sicherung:** Die acht zu r6 gehörenden Dateien wurden im Commit `0b04e2e` gezielt versioniert und erfolgreich nach `origin/main` auf GitHub übertragen. Andere vorhandene Arbeitsdateien blieben außerhalb des Commits. Die r6-Dateien liegen lokal für die manuelle Apps-Script-Übernahme bereit; eine Google-Bereitstellung und ein HQ-Live-Test stehen noch aus.

### 26.09.2026 – Firma und Ansprechpartner in ausdrücklich getrennten HQ-Aufrufen (r7)

**Live-Rückmeldung und Korrektur:** Die Firma wird in HQ angelegt, ihr Ansprechpartner fehlt. Der Nutzer hat den gespeicherten Auftragsstatus „Ausgang unklar“ mit „HQ-Schreibversuch HTTP 400“ nachgereicht. Diese alte Meldung enthält weder Endpunkt noch Validierungsdetails; die konkrete Ablehnungsursache ist daher noch nicht bewiesen. r6 führte Firma → Rücklesen → Kontakt innerhalb desselben Aufrufs aus. Die Reihenfolge war vorhanden, eine zwingende Ausführungsgrenze und Pause jedoch nicht. Der Nutzer fordert nun ausdrücklich zwei getrennte Schritte; eine zeitlich versetzte automatische Verarbeitung soll später folgen. Die alten Testaufträge sind nicht der neue Abnahmetest.

**Lokal umgesetzt:** r7 hat zwei getrennte Schaltflächen und einen expliziten serverseitigen Schrittparameter. Schritt 1 legt ausschließlich die eigene TEST-Firma an, speichert ihre HQ-ID und bestätigt Firmenname/Testzuordnung durch Rücklesen. Danach bleibt der Auftrag in „Firma bestätigt · Ansprechpartner offen“. Wiederholte Aufrufe von Schritt 1 können keinen Kontakt anlegen. Schritt 2 setzt die gespeicherte Bestätigung voraus, liest dieselbe Firmen-ID erneut, legt den Firebase-Ansprechpartner mit dieser ID an und prüft ihn zurück. Erst anschließend werden die übrigen Firmenfelder geprüft, die technische Beschreibung bereinigt und der bestätigte HQ-Stand in Firebase gespeichert. Firma und Kontakt bleiben schon vorher in Firebase verwendbar. Der Ablauf kann nach Schließen der App in einer neuen Ausführung fortgesetzt werden. Die automatische 30-Sekunden-Fortsetzung aus r6 wurde entfernt; r7 erzwingt keine Wartezeit und richtet keinen Hintergrundlauf ein.

**API-Abgleich und Fehlerdiagnose:** Die öffentliche [HQ-v2-OpenAPI-Spezifikation](https://developer.hellohq.io/swagger20.json) wurde ohne Zugangsdaten gelesen. ContactPersonPost enthält companyId und die verwendeten Kontaktfelder; daraus ergibt sich noch kein eindeutiger fehlender Pflichtwert für den alten Fehler. Leere optionale Zeichenfolgen werden jetzt ausgelassen, statt etwa eine leere E-Mail an die Validierung zu senden. Kontaktlisten verwenden die dokumentierten Parameter $filter/$top/$skip. Schreibversuche speichern Methode, Endpunkt ohne Objekt-ID, Zeitpunkt, HTTP-Status und ausschließlich bekannte, in der Fehlerantwort erkannte Feldnamen. Rohe Antworten und darin enthaltene Kundenwerte werden weder gespeichert noch an den Nutzer ausgegeben. Die Auftragsmeldung zeigt bei Fehlern den konkreten Endpunkt und diese Feldhinweise. HTTP-Fehler und verlorene Antworten geben weiterhin keine blinde Wiederholung frei. Die Abschlussprüfung toleriert wie die erste Adressprüfung eine von HQ normalisierte Adressbeschreibung, prüft aber weiterhin Anschrift und Website.

**Prüfung und Grenzen:** 53 lokale Prüfungen mit synthetischen HQ-/Firebase-Diensten bestanden. Neue Prüfungen belegen die Ausführungsgrenze, die Sperre vor Firmenbestätigung, wiederholten Schritt 1 ohne Kontakt-POST, Rückprüfung eines inzwischen geänderten Firmenziels, Fortsetzung in einem frischen Backend-Kontext allein anhand gespeicherter Daten, datensparsame HTTP-400-Diagnosen, dokumentierte Kontaktfilter sowie die getrennten UI-Aktionen. Bestehende Prüfungen zu verlorenen Antworten und doppelten Anlagen bestehen weiterhin. HTML-Skripte syntaktisch geprüft; generierte Dateien aus den Quellen erstellt. Keine echten HQ-/Firebase-Schreibaufrufe und kein Apps-Script-Upload durch Codex. Die tatsächliche Annahme des Kontakt-POSTs bleibt bis zum nächsten Nutzertest offen; eine Verzögerung allein wurde nicht als erwiesene Fehlerbehebung dargestellt.

**Übergabe:** Nur `hq-benchmark/SalesBackend.gs` und `hq-benchmark/Sales.html` vollständig übernehmen und die vorhandene Bereitstellung auf **Neue Version** setzen. Keine neuen/geänderten Skripteigenschaften, Manifest- oder Zugriffseinstellungen. Die manuelle Anleitung wurde auf r7 konsolidiert, damit veraltete Schaltflächen und widersprüchliche frühere Übertragungsschritte nicht weiter als aktuelle Anleitung erscheinen. Der vollständige Verlauf bleibt hier und in Git erhalten.

**Nächste Nutzeraktionen:**

- Die zwei Dateien nach `sales-app/MANUELL-UEBERTRAGEN.md` ersetzen und bereitstellen.
- Die Kennung `2026-09-26-r7` in der geöffneten Web-App prüfen.
- Eine neue eindeutig benannte TEST-Firma mit erfundenem Ansprechpartner in Firebase speichern.
- **1. Firma in HQ anlegen und bestätigen** starten und die Firma in HQ kontrollieren.
- Für diesen Test etwa eine Minute warten; dies ist eine bewusste Testpause, keine programmierte Frist.
- **2. Ansprechpartner nach HQ übertragen** starten und genau diesen Kontakt bei derselben HQ-Firma prüfen.
...
**Stand und Grenze:** Die acht zugehörigen Dateien wurden gezielt im Commit `2d10081` versioniert und erfolgreich nach `origin/main` auf GitHub übertragen; fremde Änderungen wurden nicht mitgenommen. Dieser Sicherungsnachtrag wird separat versioniert. Keine neuen/geänderten Skripteigenschaften, Manifest- oder Zugriffseinstellungen. Die r8-Bereitstellung und erfolgreiche Kontaktanlage bleiben bis zur Nutzerrückmeldung offen. Die übrige Roadmap einschließlich automatischer zeitversetzter Verarbeitung bleibt bestehen.

### 26.09.2026 – Ansprechpartner live angelegt; E-Mail fehlt, gezielter Lesetest r8.1

**Live-Ergebnis:** Der Nutzer bestätigt nach r8 die erfolgreiche Anlage des Ansprechpartners in HQ. Seine E-Mail-Adresse bleibt in HQ leer. Die App zeigt „Kontakt in HQ · Prüfung offen“ und eine unvollständige Rückprüfung. Damit ist die Kontaktanlage tatsächlich bestätigt, die vollständige Feldübertragung noch nicht. Die fehlende Ansprache war ein Hindernis des vorherigen POSTs; daraus folgt kein Nachweis für korrekte E-Mail-Übertragung. Die mitgesendete echte HQ-ID wird nicht dokumentiert.

**Untersuchung:** Formular, Firebase-Entwurf und Kontakt-Payload verwenden bereits `eMail`, wie es die erneut gelesene öffentliche HQ-Spezifikation für ContactPersonPost/Put/Get beschreibt. Daneben existiert `defaultAddress.email` an der Kontaktadresse. Welche Werte der konkrete Kontakt zurückliefert und ob weitere Felder abweichen, ist noch nicht bekannt. Eine optionale Rückfrage, ob die Adresse in der App sichtbar ist, ist zum Abschluss dieses Arbeitsschritts unbeantwortet. Keine Behauptung, dass ein anderer Feldname oder eine andere Schreibroute den Fehler bereits behebt. Die geplante Schreibkorrektur wurde nach Prüfung des vorhandenen Mappings zugunsten eines gezielten Lesetests zurückgestellt.

**Lokal umgesetzt:** Die vorhandene Aktion **Homepage in HQ prüfen** ergänzt unter **Ansprechpartner** eine Diagnose mit Kennung `Kontaktprüfung 2026-09-26-r8.1`. Sie prüft die gespeicherte Kontakt-ID und Firmenzuordnung und zeigt nur Präsenz/Übereinstimmung von Firebase-E-Mail, HQ eMail/email/Email/EMail sowie defaultAddress.email. Bei verknüpfter, aber nicht gelieferter Kontaktadresse versucht sie einen zusätzlichen GET mit expand=DefaultAddress; Fehlschläge bleiben als unbekannt erkennbar. Die Diagnose nennt abweichende bekannte Kontaktfelder ohne ihre Werte. Auch die normale Rückprüfungsfehlermeldung nennt jetzt die betroffenen Felder, statt alle Abweichungen pauschal zusammenzufassen. Keine neue Kontaktanlage, keine neue Schreibroute und kein geändertes E-Mail-Mapping. Anhand des Ergebnisses soll anschließend die gezielte Korrektur am bereits angelegten Kontakt umgesetzt werden.

**Prüfung:** 66 synthetische lokale Prüfungen bestanden. Neue Fälle prüfen leere/fehlende/abweichende E-Mail-Felder, alternative Feldschreibweisen, reine GET-Aufrufe der Diagnose, keine Ausgabe echter Kontaktwerte, Zuordnungssperre für fremde Kontakte, Adress-Expand samt Fehlschlag und keine doppelte Anlage bei wiederholter Rückprüfung einer fehlenden E-Mail. SalesBackend.gs neu erzeugt, Sales.html nachweislich ohne Änderung. Kein echter HQ-/Firebase-Schreibaufruf oder Apps-Script-Upload durch Codex. Eine zwischenzeitliche automatische Freigabeprüfung scheiterte wegen Workspace-Guthaben; der betroffene Schritt wurde nicht ausgeführt. Nach dem Nutzerauftrag „fahre fort“ war die erneute Prüfung erfolgreich und die Arbeit konnte fortgesetzt werden.

**Übergabe und offene Arbeit:** Nur `hq-benchmark/SalesBackend.gs` vollständig ersetzen und die bestehende Bereitstellung auf **Neue Version** setzen. Oberfläche und Backend-Kompatibilitätskennung bleiben r8; der tatsächliche Diagnosestand ist an r8.1 im Prüfergebnis erkennbar. Keine neuen/geänderten Skripteigenschaften, Manifest- oder Zugriffseinstellungen. GitHub-Sicherung folgt nach gezielter Prüfung. Die E-Mail-Korrektur ist noch offen und benötigt das Live-Leseergebnis; der Diagnoseschritt ist keine abgeschlossene Fehlerbehebung.

**Nächste Nutzeraktionen:**

- Die eine Backend-Datei nach der aktuellen manuellen Anleitung ersetzen und eine neue Version bereitstellen.
- In der Web-App **Daten-Testseite** öffnen.
