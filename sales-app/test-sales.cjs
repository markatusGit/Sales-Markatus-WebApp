...
test('creation sequences company then contact with returned ID and verifies them',()=>{const f=fixture(),r=f.ctx.saveSalesTestCompany(f.input);
  assert.equal(f.ctx.runSalesJob(r.id).state,'companyCreated');assert.equal(f.calls.filter(x=>x.method==='post').length,1);assert.match(f.remote.Companies[101].description,/\[Sales-Test/);assert.equal(f.db['sales_drafts/'+r.id].hqId,101);
  assert.equal(f.ctx.runSalesJob(r.id).state,'contactCreated');const writes=f.calls.filter(x=>x.method==='post');assert.deepEqual(writes.map(x=>x.path),['/v2/Companies','/v2/ContactPersons']);assert.equal(writes[1].body.companyId,101);assert.match(f.remote.Companies[101].description,/\[Sales-Test/);
  assert.equal(f.ctx.runSalesJob(r.id).state,'synced');assert.ok(!f.remote.Companies[101].description.includes('[Sales-Test'));assert.equal(f.db['sales_companies/101'].contacts[0].id,202);assert.ok(!('addressOrigin' in writes[0].body));});
test('HQ company read delay keeps a confirmed company phase without a second POST',()=>{const f=fixture(),r=f.ctx.saveSalesTestCompany(f.input),original=f.ctx.salesOne_;assert.equal(f.ctx.runSalesJob(r.id).state,'companyCreated');f.ctx.salesOne_=(entity,id)=>{if(entity==='Companies')throw Error('HQ noch nicht lesbar');return original(entity,id);};const retry=f.ctx.runSalesJob(r.id);assert.equal(retry.state,'companyCreated');assert.match(retry.message,/HQ noch nicht lesbar/);assert.equal(f.calls.filter(x=>x.method==='post').length,1);f.ctx.salesOne_=original;assert.equal(finishCreate(f,r.id).state,'synced');});
test('HQ contact read delay keeps a confirmed contact phase without a second POST',()=>{const f=fixture(),r=f.ctx.saveSalesTestCompany(f.input),original=f.ctx.salesOne_;assert.equal(f.ctx.runSalesJob(r.id).state,'companyCreated');assert.equal(f.ctx.runSalesJob(r.id).state,'contactCreated');f.ctx.salesOne_=(entity,id)=>{if(entity==='ContactPersons')throw Error('HQ noch nicht lesbar');return original(entity,id);};const retry=f.ctx.runSalesJob(r.id);assert.equal(retry.state,'contactCreated');assert.match(retry.message,/HQ noch nicht lesbar/);assert.equal(f.calls.filter(x=>x.method==='post').length,2);f.ctx.salesOne_=original;assert.equal(f.ctx.runSalesJob(r.id).state,'synced');});
...
test('wrong returned address pauses creation before the contact POST',()=>{const f=fixture(),r=f.ctx.saveSalesTestCompany(f.input),orig=f.ctx.salesOne_;f.ctx.salesOne_=(entity,id)=>{const v=orig(entity,id);if(entity==='Companies')v.defaultAddress.city='Wrong';return v;};assert.equal(f.ctx.runSalesJob(r.id).state,'companyCreated');const check=f.ctx.runSalesJob(r.id);assert.equal(check.state,'companyCreated');assert.match(check.message,/Standardadresse/);assert.equal(f.calls.filter(x=>x.path==='/v2/ContactPersons'&&x.method==='post').length,0);assert.equal(f.calls.filter(x=>x.path==='/v2/Companies'&&x.method==='post').length,1);});
...
test('startup guard replaces a stalled static screen with a useful release hint',()=>{
  ...
  assert.ok(root.innerHTML.includes('2026-09-25-r5'));
